...
export function ProductSearch() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/api/stats').then((response) => response.json()).catch(() => null),
      fetchProductCategories(),
    ]).then(([stats, facets]) => {
      if (cancelled) return
      setTotalProducts(typeof stats?.products === 'number' ? stats.products : 0)
  ...
  const resultSummary = useMemo(() => {
    if (!hasSearched) return `${totalProducts.toLocaleString()} products`
    if (isLoading) return 'Searching...'
    if (totalCount === 0) return 'No results'
    return `${totalCount.toLocaleString()} results`
  }, [hasSearched, isLoading, totalCount, totalProducts])
